  ...
  yield takeLatest(ActionTypes.RESTART_CHAIN,
    function* (action: ActionsOfType<typeof ActionTypes.FORK_CHAIN>): Saga {
      yield call(monitorBackendTask, function* (): Saga {
        ...
        yield call(updateSubscriptions, params);
      });
    }
  );
...
function* updateSubscriptions(params: Params): Saga {
  ...
  /* Subscribe to the visible games plus that of the selected chain. */
  const channels = new Set();
  channels.add(`contest:${params.contestId}`);
  function addChainGameKey(chain: Entity<Chain>) {
    if (chain.isLoaded && chain.value.currentGameKey !== "") {
      channels.add(`game:${chain.value.currentGameKey}`);
    }
  }
  ...
  yield put(actionCreators.eventSourceSubsChanged(Array.from(channels.keys())));
